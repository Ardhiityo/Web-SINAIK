                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="overflow-auto card">
                                    <form action="{{ route('link-productive.roles.update', ['role' => $user->id]) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                                ...
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <select id="role" required name="role" class="form-control">
                                                        <option value="admin_astra"
                                                            {{ old('role', $user->roles->pluck('name')->first()) === 'admin_astra' ? 'selected' : '' }}>
                                                            Admin Astra
                                                        </option>
                                                        <option value="admin_ico"
                                                            {{ old('role', $user->roles->pluck('name')->first()) === 'admin_ico' ? 'selected' : '' }}>
                                                            Admin Diskopukm
                                                        </option>
                                                    </select>
                                                </div>
