                ...
                @role('admin')
                    @if (!$finalProjects->isEmpty())
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form
                                    action="{{ route('periodes.export.academic-calendar', ['academic_calendar' => $academicCalendar->id, 'status' => 'final_project']) }}"
                                    method="GET">
                                    <div class="row">
                                        @if ($errors->any())
                                            <div class="col-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('periodes.final-projects', ['periode' => $academicCalendar->id]) }}"
                                    method="GET">
                                    <div class="form-group col-md-6">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            @else
                                @foreach ($finalProjects as $date => $finalProjectByDate)
                                    <div class="card-header">
                                        <h4>
                                            {{ $date }}
                                        </h4>
                                    ...
                                    <div class="overflow-auto card-body">
                                        <table class="table table-bordered">
                                            ...
                                            <tbody>
                                                @foreach ($finalProjectByDate as $finalProject)
                                                    <tr class="text-nowrap">
                                                        <th scope="row">{{ $loop->iteration }}</th>
                                                        <td>{{ $finalProject->student->nim }}</td>
                                                        <td>{{ $finalProject->student->name }}</td>
                                                        <td>{{ $finalProject->session_time }}</td>
                                                        <td>{{ $finalProject->room->name }}</td>
                                                        <td>{{ $finalProject->student->lecture1->name }}</td>
                                                        <td>{{ $finalProject->student->lecture2->name }}</td>
                                                        @role('admin')
                                                            <td>{{ $finalProject->examiner1->name }}</td>
                                                            <td>{{ $finalProject->examiner2->name }}</td>
                                                            <td>{{ $finalProject->moderator->name ?? '-' }}</td>
                                                            <td>
                                                                <a href="{{ route('final-projects.edit', ['final_project' => $finalProject->id]) }}"
                                                                    class="btn btn-warning">Edit</a>
                                                                <form id="form-delete"
                                                                    action="{{ route('final-projects.destroy', ['final_project' => $finalProject->id]) }}"
